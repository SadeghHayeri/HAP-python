...
class NodeMCUSwitch(Accessory):
    ...
    def __init__(self, *args, URL, switchIndex, **kwargs):
        super(NodeMCUSwitch, self).__init__(*args, **kwargs)
    ...
    def _set_services(self):
        super(NodeMCUSwitch, self)._set_services()
    ...
    def stop(self):
        super(NodeMCUSwitch, self).stop()

    def run(self):
        while not self.run_sentinel.wait(1):
            r = requests.get(self.statusURL)
            if '1' in r.text:
                self.switchState.set_value(True)
            elif '0' in r.text:
                self.switchState.set_value(False)
            else:
                print("error!", r.text)
